    ...
    var pg = {
      ...
      loadZoomImg:function(idx){
        if($('#pgzoomimg' + idx).length === 0){
          ...
          image = $('#pgzoomimg' + idx);
          if(pg.videoArray[idx])
          {
            image.after('<iframe style="vertical-align:middle;max-width: 80%;max-height: 75%;" id="ytplayer" type="text/html" width="640" height="390"  src="http://www.youtube.com/v/'+pg.videoArray[idx]+'?version=3&enablejsapi=1&autoplay=0"  frameborder="0"/>');
          }
